...
class BinanceHttp(object):
    ...
    def get_realTimeFundRate(self, symbol):
        """
        Futures only
        """
        if self.type != 'Spot':
            path = '/fapi/v1/premiumIndex' if self.type == 'USDTFutures' else '/dapi/v1/premiumIndex'
            params = {
                    "symbol":symbol
                    }        
            ...
            raise Exception('parameter set error SPOT!!!')

    def get_historyFundRate(self, symbol, start_time=None, end_time=None, limit=1000):
        """
        Futures only
        """    
        if self.type != 'Spot':    
            path = '/fapi/v1/fundingRate' if self.type == 'USDTFutures' else '/dapi/v1/fundingRate'
            params = {
                    "symbol":symbol,
                    "limit":limit
                    }        
